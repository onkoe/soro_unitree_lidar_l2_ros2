...
UnitreeLidarSDKNode::UnitreeLidarSDKNode(const rclcpp::NodeOptions& options) :
    ...
    if (initialize_type_ == 1) {
        lsdk_->initializeSerial(
            serial_port_,
            baudrate_,
            cloud_scan_num_,
            use_system_timestamp_,
            range_min_,
            range_max_
        );
...
void UnitreeLidarSDKNode::timer_callback() {
    ...
    static pcl::PointCloud<PointType>::Ptr cloudOut(
        ...
        if (lsdk_->getPointCloud(cloud)) {
            ...
            rclcpp::Time timestamp(
                ...
                static_cast<uint32_t>(
                    (cloud.stamp - static_cast<int32_t>(cloud.stamp)) * 1e9
                )
            );
